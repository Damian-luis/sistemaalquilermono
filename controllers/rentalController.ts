...
import { addRental, getRentalById, getRentals, updateRental, deleteRental, getRentalsByUserDni,updateRentalForDevolution } from "../services/rentalService";
...
async function makeDevolutionC(req: Request, res: Response): Promise<void> {
  const renta = req.body.devolution
  try {
    const rental = await updateRentalForDevolution(renta);
    if (rental) {
      res.status(HTTP_CODES.SUCCESS).json({ rental });
    } else {
      res.status(HTTP_CODES.NOT_FOUND).json({ error: RESPONSE_MESSAGES.RENTAL_NOT_FOUND });
...
export { addRentalC, getRentalByIdC, getRentalsC, updateRentalC, deleteRentalC,getRentalByDniC,makeDevolutionC };
